  ...
  items.forEach((item) => {
    if (
      ...
      item.querySelector(".edit-item").addEventListener("click", function () {
        addBtn.innerHTML = "Editar Tarefa";
      ...
      item.querySelector(".delete-item").addEventListener("click", function () {
        if (confirm("Você tem certeza que quer remover essa tarefa?")) {
          itemList.removeChild(item);
...
form.addEventListener("submit", function (e) {
  ...
  if (itemName.length === 0) {
    sendFeedback("Adicione uma tarefa valida", `${bootstrap ? "danger" : "red"}`);
  } else {
    addBtn.innerHTML = "Adicione uma tarefa";
...
clearButton.addEventListener("click", function () {
  confirm("Você tem certeza que quer limpar a lista?")
...
function sendFeedback(text, className) {
  if (bootstrap) {
    ...
    setTimeout(function () {
      ...
      feedback.innerHTML = "Escreva uma tarefa";
    }, 3000);
  } else if (tailwindCSS) {
    ...
    setTimeout(function () {
      ...
      feedback.innerHTML = "Esreva uma tarefa";
    }, 3000);
  } else {
